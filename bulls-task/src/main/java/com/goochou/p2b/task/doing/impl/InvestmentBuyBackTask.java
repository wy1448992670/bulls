 ...
 * @version        1.0
 */
public class InvestmentBuyBackTask extends BaseTask {

	private static final long serialVersionUID = 2553116224855470049L;
	private static final Logger logger = Logger.getLogger(InvestmentBuyBackTask.class);

    @Override
    protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
    	logger.info("牛只投资订单自动回购 start------");
    	...
    	try {
        	dueInvestmentList=investmentService.getDueInvestmentCouldAutoBuyBack();
        } catch (Exception e) {
            e.printStackTrace();
        ...
        for(Investment dueInvestment:dueInvestmentList) {
        	try {
        		logger.info(dueInvestment.getOrderNo()+"牛只投资订单自动回购");
    			investmentService.doInvestmentBuyBack(dueInvestment);
    			
    			logger.info(dueInvestment.getOrderNo()+"牛只投资订单自动回购成功");
			} catch (Exception e) {
				e.printStackTrace();
				logger.error(e.getMessage(), e);
			}
