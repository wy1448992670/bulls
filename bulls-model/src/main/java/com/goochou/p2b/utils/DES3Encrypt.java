...
public class DES3Encrypt {
	...
	public static String encrypt(String xmlStr) {
		...
		byte[] md5Hasn = null;
		try {
			md5Hasn = DES3Encrypt.MD5Hash(encrypt, 0, encrypt.length);
		} catch (Exception e) {
			e.printStackTrace();
		}
		// 组合消息体
		byte[] totalByte = DES3Encrypt.addMD5(md5Hasn, encrypt);

		// 取密钥和偏转向量
		byte[] key = new byte[8];
		byte[] iv = new byte[8];
		getKeyIV(DES3Encrypt.key, key, iv);
		...
		byte[] temp = null;
		try {
			temp = DES3Encrypt.DES_CBC_Encrypt(totalByte, deskey, ivParam);
		} catch (Exception e) {
			e.printStackTrace();
		}

		// 使用Base64加密后返回
		return new BASE64Encoder().encode(temp).replaceAll("\\r?\\n", "");
	}
	...
	public static String decrypt(String xmlStr) {
		...
		byte[] temp = null;
		try {
			temp = DES3Encrypt.DES_CBC_Decrypt(encBuf, deskey, ivParam);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		byte[] md5Hash = null;
		try {
			md5Hash = DES3Encrypt.MD5Hash(temp, 16, temp.length - 16);
		} catch (Exception e) {
			e.printStackTrace();
		}
		String result = null;
	...
	public static void main(String[] args) throws Exception {
		String encry = DES3Encrypt.encrypt("附件一：还款计划");
		System.out.println(encry);
		System.out.println("=================");
		System.out.println(DES3Encrypt.decrypt(encry));
		
		System.out.println(DES3Encrypt.encrypt("转让人：（电子签章）"));
		System.out.println(DES3Encrypt.encrypt("受让人：（电子签章）"));
