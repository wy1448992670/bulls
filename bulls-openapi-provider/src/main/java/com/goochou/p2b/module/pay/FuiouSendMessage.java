...
import com.goochou.p2b.invocation.impl.FuiouSendMessageInvocation;

@Service
public class FuiouSendMessage extends BaseAO implements HessianInterface
{
	
	@Autowired
	private FuiouSendMessageInvocation fuiouSendMessageInvocation;
	
	@Override
    public Response execute(ServiceMessage msg)
    {
    	FuiouSendMessageRequest req = (FuiouSendMessageRequest) msg.getReq();
		FuiouDataResponse response = new FuiouDataResponse();
		try {
			response = fuiouSendMessageInvocation.executeService(req);
		} catch (Exception e) {
