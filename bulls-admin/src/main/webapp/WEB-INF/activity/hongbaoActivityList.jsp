                                ...
                                <div class="form-group col-md-3">
                                    <shiro:hasPermission name="user:export:app">
                                        <a style="margin-left: 10px;" class="btn btn-sm btn-primary-outline pull-right hidden-xs"
                                           href="${basePath}report/hongbaoReport?activityId=${activityId}&keyword=${keyword}&keyword1=${keyword1}&startTime=<fmt:formatDate value="${startTime }" pattern="yyyy-MM-dd"/>&endTime=<fmt:formatDate value="${endTime }" pattern="yyyy-MM-dd"/>"
                            ...
                            <div class="row">
                                <div class="form-group col-md-6">
                                    <input class="form-control datepicker" id="startTime" name="startTime" type="text"
                                           placeholder="请选择红包发送开始时间" value="<fmt:formatDate value="${startTime }" pattern="yyyy-MM-dd"/>"/>
                                </div>
                                <div class="form-group col-md-6">
                                    <input class="form-control datepicker" id="endTime" name="endTime" type="text"
                                           placeholder="请选择红包发送结束时间" value="<fmt:formatDate value="${endTime }" pattern="yyyy-MM-dd"/>"/>
                                </div>
                            </div>
                        ...
                        <table class="table table-bordered table-hover">
                            ...
                            <c:forEach var="i" items="${list }" varStatus="s">
                                <tr>
                                    ...
                                    <td>
                                        <fmt:formatDate value="${i.send_time}" pattern="yyyy-MM-dd"/>
                                    </td>
                                    <td>
                                            ${i.amount }
                                    </td>
    ...
    $(function () {
        ...
        $('#pagination').bootstrapPaginator({
            ...
            pageUrl: function (type, page, current) {
                return "hongbaoActivityList?activityId=${activityId}&keyword=${keyword}&keyword1=${keyword1}&startTime=<fmt:formatDate value="${startTime }" pattern="yyyy-MM-dd"/>&endTime=<fmt:formatDate value="${endTime }" pattern="yyyy-MM-dd"/>&page=" + page;
            }
        });
        var activityOption = {
            placeholder: "请选择活动标题",
