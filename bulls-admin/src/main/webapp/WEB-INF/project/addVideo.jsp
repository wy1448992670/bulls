                            ...
                            <div class="form-group">
								...
								<div class="col-md-7"> 
									<input class="form-control" style="width:300px;" id="showTime" name="showTime"
									 placeholder="显示时间" class="Wdate" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" />
								</div>
							</div>
                            <div class="form-group">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <!--       <div class="form-group">
                        ...
                        <div class="col-md-7">
                            <input class="form-control" name="picName" id="picName2" type="text" placeholder="视频名称" />
                        </div>
                    </div>
 -->
                    <div class="form-group">
                        ...
                        <div class="col-md-3">
                            ...
                            <video  controls="controls" autoplay="autoplay"  muted="true"  src="" id="target2"  width="200px;"/>
...
<script type="text/javascript">
    $(function () {
        ...
        $(".upload-picture-1").on("click", ".icon-trash", function () {
            var $this = $(this);
            if (confirm("您确定要删除该视频吗?视频一旦删除，将不可恢复!")) {
                var picId = $this.parent().parent().attr("picId");
                $.ajax({
                    url: "delete/picture?id=" + picId,
                    type: "post",
            ...
            .fileupload(
                {
                    ...
                    add: function (e, data) {
                    	var file = data.files[0];
                        if (!new RegExp(/(\.|\/)(mp4|webm|ogg)$/i).test(file.type)) {
                        	alert("错误的视频类型");
                            $(".alert-danger .alert-content").text("错误的视频类型");
                            $(".alert-danger").fadeIn().delay(2000).fadeOut();
                            return false;
                        }
                        if (file.size > 20971520) {// 20M
                        	alert("视频大于20M");
                            $(".alert-danger .alert-content").text("视频大于20M");
                            $(".alert-danger").fadeIn().delay(2000).fadeOut();
                            return false;
                        }
                        $('#target2').show();
                    ...
                    added: function (e, data) {
                        ...
                        if (upload.status == "error") {
                            ...
                            $(".alert-danger .alert-content").text(upload.message);
                            $(".alert-danger").fadeIn().delay(2000).fadeOut();
                        } else {
                            var $html = $('<a class="gallery-item fancybox" rel="g1" title="" picId="">'
                                + '<video style="width:20%;" controls="controls" autoplay="autoplay"  muted="true" src="" />' + '<div class="actions">'
                                + '<i class="icon-trash"></i><i class="icon-zoom-in"></i>' + '</div>'
                                + '</a>');
                            var path = data.video.videoUrl;
                            $html.attr("href", path).attr("videoId",
                                data.video.id);
                            $html.find("video").attr("src", path);
                            ...
                            $("#validate-form")
                                .append(
                                    '<input id="video-' + data.video.id + '" type="hidden" name="videoIds"  value="' + data.video.id + '"/>');
                        }
                    }
                });
    });
    ...
    $('#validate-form').submit(function (event) {
    	...
    	var videoIds =$("input[name='videoIds']").val();
        ...
        $.ajax({ 
            ...
            success: function (data) {
                ...
                if (obj.code == "1") {
                    alert("上传成功！");
                    window.location.href="videoAlbumList";
                } else {
                   alert("上传失败！");
                }
            }
